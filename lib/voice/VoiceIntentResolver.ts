...
function extractFieldValues(
  ...
  for (const { field, alias } of FLAT_ALIASES) {
    ...
    const pat = new RegExp(
      `(?:change|update|set|make|ko)\\s+(?:the\\s+)?(${alias.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})\\s+(?:to|into|ko|mein|banao|kardo|kar do)\\s+["'\`]?([A-Za-z0-9][A-Za-z0-9\\s.,_-]{0,60})`,
      'i',
    );
    ...
    const toPat = new RegExp(
      `${alias.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s+(?:to|ko|banaye|banao|badlo|kardo|kar do)\\s+["'\`]?([A-Za-z0-9][A-Za-z0-9\\s.,_-]{0,60})`,
      'i',
    );
  ...
  if (state) {
    ...
    for (const key of known) {
      ...
      const genericPat = new RegExp(
        `(?:change|update|set)\\s+(?:the\\s+)?${key}\\s+(?:to|into)\\s+["'\`]?([A-Za-z0-9][A-Za-z0-9\\s.,_-]{0,60})`,
        'i',
      );
...
function clarify<T extends SemanticState>(
  text: string,
  fallback: Candidate[],
): ResolutionResult<T> {
  if (!fallback.length) {
...
export class VoiceIntentResolver<T extends SemanticState = SemanticState> {
  ...
  resolve(
    ...
    if (!transcript)
      return { kind: 'unsupported', reason: 'Enter a voice instruction.' };
    const text = transcript.toLowerCase();
    const active = checkpoints.find((c) => c.id === activeCheckpointId) ?? null;

    if (
      /^(undo|undo that|wapas|wapas karo|wapas le lo|revert|u?ndo\s+that\.?|actually,?\s*undo|udne do|phir se pehle)[.!]?$/i.test(
        text,
      )
    ) {
    ...
    if (
      /undo\s+(?:the\s+)?last|last\s+(?:action|operation|step|change|kaam)\s+undo/i.test(
        text,
      )
    ) {
      return { kind: 'resolved', operation: { type: 'UNDO' } };
    ...
    // The demo graph is seeded with the initial plan so reset is deterministic. Treat a
    // spoken restatement of that plan as selecting the matching root checkpoint.
    if (
      active &&
      /\b(?:plan|create|make)\b.*\btrip\b.*\bgoa\b/i.test(text) &&
      /\b(?:forty|40)\b/i.test(text)
    ) {
      const matching = checkpoints.find((checkpoint) => {
        const state = checkpoint.structuredState as Record<string, unknown>;
        return (
          String(state.destination).toLowerCase() === 'goa' &&
          state.budget === 40000 &&
          state.duration === 5
        );
      });
      if (matching)
        return {
          kind: 'resolved',
          operation: { type: 'SWITCH_CHECKPOINT', checkpointId: matching.id },
        };
    }

    if (
      /compare|mukabla|muqabla|kya antar hai|difference|kya farak hai|compare karo|tulna|what changed|kya badla/i.test(
        text,
      )
    ) {
      if (!active)
      ...
      if (/^(?:what changed|kya badla)[?.!]*$/i.test(text) && active.parentId) {
        return {
          kind: 'resolved',
          operation: {
            type: 'COMPARE',
            fromCheckpointId: active.parentId,
            toCheckpointId: active.id,
          },
        };
      }
      const candidates = this.refs.resolve(
        text,
        checkpoints,
        activeCheckpointId,
        { excludeActive: true },
      ...
      if (candidates.length === 1) {
        return {
          ...
          operation: {
            type: 'COMPARE',
            fromCheckpointId: candidates[0].id,
            toCheckpointId: active.id,
          },
        };
      }
      const others = checkpoints.filter((c) => c.id !== activeCheckpointId);
      const fallback = others.map((c) => ({
        id: c.id,
        label: c.label,
        versionNumber: c.versionNumber,
        whyMatched: ['non-active'],
      }));
      if (candidates.length > 1)
        return clarify(
          'Which version should I compare with the active one?',
          candidates,
        );
      if (others.length === 1)
        return {
    ...
    if (
      /go back one step|rewind one step|ek step peeche|ek kadam peeche|peeche ek step/i.test(
        text,
      )
    ) {
    ...
    const switchLike =
      /go back|switch|return to|continue from|use (?:the\s+)?(?:previous|active|that|this|other|original)|wapas jao|pe wapas|pe le jao|le jao|dikhao|dikhaye|use karo|select karo|chuno|chuniye|activate|chaloo|chalao|open karo/i.test(
        text,
      );

    if (switchLike) {
      const result = this.refs.resolveOne(
        text,
        checkpoints,
        activeCheckpointId,
      );
      if (result.kind === 'exact') {
        return {
          kind: 'resolved',
          operation: {
            type: 'SWITCH_CHECKPOINT',
            checkpointId: result.candidate.id,
          },
        };
      }
      ...
      const all = checkpoints.map((c) => ({
        id: c.id,
        label: c.label,
        versionNumber: c.versionNumber,
        whyMatched: ['all-checkpoints'],
      }));
      if (!checkpoints.length)
        return {
          kind: 'unsupported',
          reason: 'No checkpoints exist to switch to.',
        };
      return clarify('Which checkpoint do you mean?', all);
    }

    const mergeLike =
      /(?:take|merge|copy|bring|le lo|le aao|laao|lao|use karo|include|add)\s+(?:the\s+)?([\w\s-]*?)\s*(?:from|se)\s+/i.test(
        text,
      ) ||
    ...
    if (mergeLike) {
      if (!active)
        return {
          kind: 'unsupported',
          reason: 'Create a checkpoint first before merging fields.',
        };
      const src = this.refs.resolveOne(text, checkpoints, activeCheckpointId, {
        excludeActive: true,
      });
      const specifiedFields = detectFields(text);
      if (
        specifiedFields.length === 0 &&
        /hotel|stay|room|accommodation/i.test(text)
      )
        specifiedFields.push('accommodation');
      if (specifiedFields.length === 0) specifiedFields.push('accommodation');
      const keepRest =
        /but\s+(?:keep|don.?t change|don't change|dont change|leave|mat karna|change mat karna|baaki|baki)/i.test(
          text,
        ) ||
        ...
        return {
          ...
          operation: {
            ...
            meta: makeMeta(
              label,
              transcript,
              `Selective ${specifiedFields.join(', ')} merge`,
            ),
          },
        };
      }
      if (src.kind === 'ambiguous') {
        return clarify(
          'Which version should supply the fields?',
          src.candidates,
        );
      }
      const others = checkpoints.filter((c) => c.id !== activeCheckpointId);
    ...
    const forkLike =
      /another version|new version|fork|try another|naya version|naya banao|banao|banaye|ek aur|aur ek|new branch|dobra|dubara|once more|phir se ek|alternative|variant/i.test(
        text,
      );

    if (forkLike) {
      const baseSrc = this.refs.resolveOne(
        text,
        checkpoints,
        activeCheckpointId,
      );
      ...
      if (!baseId)
        return {
          kind: 'unsupported',
          reason: 'Create a checkpoint first before forking.',
        };
      const base = checkpoints.find((c) => c.id === baseId)!;
      const changes = extractChanges(text, base.structuredState) as Partial<T>;
      const budgetLabel = changes.budget
        ? `₹${Math.round(Number(changes.budget) / 1000)}k`
        : '';
      const priorityLabel =
        (changes.priorities as string[] | undefined)?.[0] ?? '';
      const titleParts = [budgetLabel, priorityLabel].filter(Boolean);
      const label = titleParts.length
        ? titleParts.join(' ')
        : `Fork of ${base.label}`;
      return {
        kind: 'resolved',
    ...
    const updateLike =
      /change (?:the\s+)?|update (?:the\s+)?|set (?:the\s+)?|make (?:the\s+)?|kar do|kardo|banao|banaye|badlo|bada do|chhota karo|kam karo|zyada karo|increase|decrease|badao|ghatao|badal do|modify|\bko\b.*\b(?:kar do|kardo|banao|banaye|badlo|change)\b/i.test(
        text,
      );
    ...
    if (updateLike) {
      if (!active)
        return {
          kind: 'unsupported',
          reason: 'Create a checkpoint first before updating state.',
        };
      const changes = extractChanges(
        text,
        active.structuredState,
      ...
      if (!Object.keys(changes).length) {
        return {
          kind: 'unsupported',
          reason: `I could not identify which fields to change from: ${transcript}`,
        };
      }
      const labelParts: string[] = [];
      if ('budget' in changes && typeof changes.budget === 'number')
        labelParts.push(`₹${Math.round(Number(changes.budget) / 1000)}k`);
      if ('duration' in changes && typeof changes.duration === 'number')
        labelParts.push(`${changes.duration}d`);
      if ('priorities' in changes && Array.isArray(changes.priorities))
        labelParts.push((changes.priorities as string[]).join('/'));
      const label = labelParts.length
        ? `${active.label} · ${labelParts.join(' · ')}`
    ...
    return {
      kind: 'unsupported',
      reason: `That instruction does not map to a Phase 2 state operation: ${transcript}`,
    };
  }
}
