...
export const INITIAL_DEMO_INSTRUCTION = 'Plan a five-day trip to Goa for forty thousand rupees.';

function buildInitialSnapshot(): GraphSnapshot<TripState> {
  const g = new StateGraph<TripState>();
  g.create(practicalTrip, { label: '₹40k practical', userInstruction: INITIAL_DEMO_INSTRUCTION, summary: 'Five days in Goa with a value-first plan.' });
  const snap = g.export();
...
export const INITIAL_DEMO_SNAPSHOT: GraphSnapshot<TripState> = buildInitialSnapshot();

export function createDemoGraph(): StateGraph<TripState> {
  const graph = new StateGraph<TripState>();
  graph.create(practicalTrip, { label: '₹40k practical', userInstruction: INITIAL_DEMO_INSTRUCTION, summary: 'Five days in Goa with a value-first plan.' });
  return graph;
}
...
export const DEMO_SEQUENCE: ReadonlyArray<{ label: string; transcript: string }> = [
  { label: 'Step 1: initial plan', transcript: 'Plan a five-day trip to Goa for forty thousand rupees.' },
  { label: 'Step 2: fork luxury', transcript: 'Make another version assuming I can spend sixty thousand rupees, stay at Taj Fort Aguada, take a flight, and prioritize comfort.' },
  ...
  { label: 'Step 5: selective merge hotel', transcript: "Take the hotel from the luxury version but don't change anything else." },
  { label: 'Step 6: what changed', transcript: 'Compare this with the previous version.' },
